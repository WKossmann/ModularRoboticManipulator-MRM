...
public:
    ...
    bool readPort(unsigned char *buf, int Nmax, int &res) {
#if 0
        if (asyncmode) {
            if (wait_flag == false) {
            ...
            tcgetattr(fd, &newtio); /* get the termio information */
            newtio.c_cflag &= ~HUPCL; /* clear the HUPCL bit */
            tcsetattr(fd, TCSANOW, &newtio); /* set the termio information */
            ...
            if (0) /* set the DTR line */
                status &= ~TIOCM_DTR;
            else
                status |= TIOCM_DTR;

            if (1) /* set the RTS line */
                status &= ~TIOCM_RTS;
            else
                status |= TIOCM_RTS;
